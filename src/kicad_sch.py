...
def convert_kicad_local_coor(pt, localorigin, orientation):
	K = 10
	x = 0
	y = 0
	if (orientation=="R0"):
		x = pt[0]*K # OK
		y = -pt[1]*K + 2*(localorigin[1] - -pt[1]*K) # OK
	elif (orientation=="R180"):
		x = pt[0]*K - 2*(localorigin[0] - pt[0]*K) # OK
		y = localorigin[1] - 2*(localorigin[1] - -pt[1]*K) # OK
	elif (orientation=="MY"):
		x = pt[0]*K + 2*(localorigin[0] - pt[0]*K) # OK
		y = -pt[1]*K + 2*(localorigin[1] - -pt[1]*K) # OK
	elif (orientation=="MX"):
		...
		x = localorigin[0] + (localorigin[1] - -pt[1]*K) # OK
		y = localorigin[1] + (localorigin[0] - pt[0]*K) # OK
	elif (orientation=="R270"):
		x = localorigin[0] - (localorigin[1] - -pt[1]*K) # OK
		y = localorigin[1] - (localorigin[0] - pt[0]*K) # OK
	elif (orientation=="MYR90"):
		x, y = convert_kicad_local_coor([x/K, -y/K], localorigin, "MY") #OK
		x, y = convert_kicad_local_coor([x/K, -y/K], localorigin, "MX") #OK
		x, y = convert_kicad_local_coor(pt, localorigin, "R270") #OK
		y = localorigin[1] + (localorigin[0] - pt[0]*K) # OK
	elif (orientation=="MXR90"):
		x, y = convert_kicad_local_coor([x/K, -y/K], localorigin, "MX") #OK
		x, y = convert_kicad_local_coor(pt, localorigin, "R90") #OK
		y = localorigin[1] - (localorigin[0] - pt[0]*K) # OK

	return [x, y]

def convert_kicad_orientation_to_local_fieldhv(prop_orientation, shape_orientation):
	orientation = "H"
	horizontal_set = {'R0', 'R180', 'MY', 'MX'}
	vertical_set = {'R90', 'R270', 'MYR90', 'MXR90'}

	if (prop_orientation in vertical_set and shape_orientation in vertical_set):
		orientation="H"
	elif (prop_orientation in horizontal_set and shape_orientation in vertical_set):
		orientation="V"
	elif (prop_orientation in vertical_set and shape_orientation in horizontal_set):
		orientation="V"
	"""
	if (prop_orientation in horizontal_set):
		orientation = "H"
